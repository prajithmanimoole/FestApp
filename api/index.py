...
def api_complete_remove_user(user_id):
    ...
    try:
        ...
        # Simple approach - do a series of DELETE statements with proper error handling
        try:
            # Handle teams related cleanup
            conn.execute('UPDATE users SET game_id = NULL, team_id = NULL WHERE team_id IN (SELECT id FROM teams WHERE leader_user_id = ?)', (user_id,))
            conn.execute('DELETE FROM team_members WHERE team_id IN (SELECT id FROM teams WHERE leader_user_id = ?)', (user_id,))
            conn.execute('DELETE FROM teams WHERE leader_user_id = ?', (user_id,))
            ...
            conn.execute('DELETE FROM allowed_users WHERE phone = ?', (phone,))
